...
class GeneralTextClassification(clsf_task._ClassificationTask):
    ...
    def _generate_feature_extraction_pipeline(self):
        ...
        if prep_params.charngramrange:
            charngram_vect_name = "charngram_vect"
            charngramvect = TfidfVectorizer(analyzer='char_wb', ngram_range=prep_params.charngramrange, lowercase=False)
                      
            text_features.append([charngram_vect_name, charngramvect])
            text_weights[charngram_vect_name] = 1
