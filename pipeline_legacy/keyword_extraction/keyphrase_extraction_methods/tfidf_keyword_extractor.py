...
from language_tools import stemmers
...
def score_keyphrases_by_tfidf(texts, lang, candidate_type='chunks', stem=True, top_n_phrases=10):
    ...
    if candidate_type == 'chunks':
        ...
        # strip punctuation?
        if stem:
            boc_texts = [stemmers.stem_words(boc_text, lang) for boc_text in boc_texts]
    
    elif candidate_type == 'all':
        unigrams = [extract_candidate_words(text, lang) for text in texts]
        if stem:
            unigrams = [stemmers.stem_words(word, lang) for word in unigrams]
...
def score_keyphrases_by_tfidf_single(text, lang, 
                                     candidate_type='all', stem=True, top_n_phrases=50):
...
def extract_keyphrases_tfidf_single(text, lang, candidate_type="all", top_n_phrases=50):       
    doc_topphrases = score_keyphrases_by_tfidf_single(text, lang, candidate_type, top_n_phrases)
    phrases = [w for w,score in doc_topphrases]
    return phrases
    
    '''
    from pprint import pprint
    pprint(doc_topphrases)
    print_top_phrases(doc_topphrases, print_weight=True)
    '''
...
def extract_keyphrases_tfidf(texts, lang, candidate_type="all", top_n_phrases=15):
    ...
    phrases = extract_keyphrases_tfidf_single(docs[0], lang)
    print(phrases)
    
    extract_keyphrases_tfidf(docs, lang)
